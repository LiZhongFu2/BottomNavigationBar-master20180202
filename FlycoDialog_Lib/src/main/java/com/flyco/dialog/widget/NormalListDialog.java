...
public class NormalListDialog extends BaseDialog<NormalListDialog> {
    ...
    private int mTitleBgColor = Color.parseColor("#ffffff");//白色
    /** title text(标题) */
    private String mTitle = "提示";
    /** title textcolor(标题颜色) */
    private int mTitleTextColor = Color.parseColor("#fdf6b318");//橘黄色
    ...
    private int mDividerColor = Color.parseColor("#e1e1e1");
    ...
    private int mItemTextColor = Color.parseColor("#303030");//黑色
    ...
    class ListDialogAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            final DialogMenuItem item = mContents.get(position);//通过识别id找到具体的Item

            LinearLayout llItem = new LinearLayout(mContext);
            llItem.setOrientation(LinearLayout.HORIZONTAL);//设各item中各组件的布局
            llItem.setGravity(Gravity.CENTER_VERTICAL);

            ImageView ivItem = new ImageView(mContext);
            ivItem.setPadding(0, 0, dp2px(15), 0);//设置图片的边距
